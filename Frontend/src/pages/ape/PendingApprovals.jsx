    ...
    try {
      ...
      toast.success(
        actionModal.action === 'APPROVED'
          ? 'Reimbursement approved and settled successfully!'
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-white">Accounts Payable Queue</h1>
        <p className="text-gray-400 text-sm mt-1">
          Review, approve, and settle reimbursement requests that have been cleared by Reporting Managers.
        </p>
      </motion.div>
      ...
      >
        {loading ? (
          ...
          <EmptyState
            title="Queue empty"
            description={search ? 'Try a different search query.' : 'No RM-approved requests pending accounts payable clearance.'}
            icon={CheckSquare}
          />
        ) : (
          <>
      ...
      <Modal
        ...
        title={actionModal?.action === 'APPROVED' ? 'Confirm Payment Approval' : 'Reject Reimbursement'}
        size="sm"
      >
        <p className="text-gray-300 text-sm mb-6">
          Are you sure you want to{' '}
          <span className={actionModal?.action === 'APPROVED' ? 'text-emerald-400 font-semibold' : 'text-red-400 font-semibold'}>
            {actionModal?.action === 'APPROVED' ? 'approve and settle' : 'reject'}
          </span>{' '}
        ...
        <div className="flex gap-3">
          ...
          <Button
            ...
            className="flex-1"
          >
            {actionModal?.action === 'APPROVED' ? 'Approve & Pay' : 'Reject'}
          </Button>
        </div>
      </Modal>
    </div>
